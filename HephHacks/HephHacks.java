 ...
 * HephHacks.java: Stress and security testing for Aegis
 *
...
public class HephHacks {
			...
			case 2: //Spam an anonymous connection
				try {
					...
					do {
						...
						clean = in.nextLine();
						if (clean.equals("y") || clean.equals("n")) {
							again = false;
						}
	...
	private static void SpamX(String user, String pass, String clean) throws UnknownHostException, IOException {
		...
		if (x != 0) {
			...
			System.out.println("Drop rate: none");
		}
			...
			if (result.equals("Success")) {
				...
				if (flag) {
					...
					for (int m = 1; m <= x; m++) {
						...
						if (mesgSpamIn.readUTF().equals("here be dragons")) {
							passed++;
						} else {
							fail++;
						}
